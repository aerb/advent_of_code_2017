...
fun parseInput(s: String): List<List<String>> =
    s.reader().readLines()
        .filter { it.isNotBlank() }
        .map { it.trim().split(Regex("\\s+")) }

fun countValid(s: String): Int {
    return parseInput(s).filter { row ->
        row.groupBy { it }.all { it.value.size == 1 }
    }.size
}

fun countValid2(s: String): Int {
    return parseInput(s).filter { row ->
        row.groupBy { it.toList().sorted() }.all { it.value.size == 1 }
    }.size
}
